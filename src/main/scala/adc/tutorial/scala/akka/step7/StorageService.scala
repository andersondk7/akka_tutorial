...
import Echo7.{ContentWritten, StorageLength}
...
class StorageService(val fileName: String) {
  ...
  /**
    ...
    * @return nothing
    */
  def postContent(content: String): Future[Try[ContentWritten]] = Future {
  ...
  def storageSize: Future[Try[StorageLength]] = Future {
    withFile[StorageLength](fileName) { f => {
      StorageLength(f.length)
    }}
  }

  def resetStorage(): Future[Try[Boolean]] = Future {
    withFile[Boolean](fileName) { f => {
      f.delete()
    }}
  }

  def forceFailure(): Future[Try[Unit]] = Future {
    Failure(PersistenceException(destination=fileName, reason=new Exception("expected")))
  }
}
